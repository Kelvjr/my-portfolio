...
export function createGradient(
  ...
  try {
    ...
    let width = Math.round((parentSection.clientWidth || innerWidth) * 0.5 * dpr);
    let height = Math.round((parentSection.clientHeight || innerHeight) * 0.5 * dpr);
    ...
    parentSection.addEventListener(
      ...
      (e) => {
        ...
        mouseY = height - ((e.clientY - rect.top) * height) / rect.height;
    ...
    function animate() {
      ...
      if (
        document.hidden ||
        !visible ||
        parentSection.dataset.gradientActive === "false" ||
        (reduced && frameCount > 0)
      )
        return;
    ...
    window.addEventListener(
      ...
      () => {
        const dpr = Math.min(window.devicePixelRatio, 1.5);
      const w = (width = Math.round(
          (parentSection.clientWidth || innerWidth) * 0.5 * dpr,
        ));
        const h = (height = Math.round(
          (parentSection.clientHeight || innerHeight) * 0.5 * dpr,
        ));
    ...
    animate();
    return () => disposals.reverse().forEach((dispose) => dispose());
  } catch (error) {
    disposals.reverse().forEach((dispose) => dispose());
    console.warn("Interactive gradient is unavailable:", error);
    return () => {};
  }
}
